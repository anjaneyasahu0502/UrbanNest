...
const fce = process.env.FCE_API_KEY
  ? new FreecustomEmailClient({ apiKey: process.env.FCE_API_KEY })
  : null;

test.describe('Signup with Email OTP Verification', () => {
  let testInbox;
  let testUsername;
  ...
  test('should prevent login before verification', async ({ page }) => {
    ...
    const errorMessage = page.locator('.alert-danger');
    await expect(errorMessage).toBeVisible();
    await expect(errorMessage).toContainText(/verify.*email/i);
  });
});
